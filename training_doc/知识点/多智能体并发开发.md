...
> **并发的关键**：Step 5 中两个终端同时启动，互不等待——这就是并发。Worktree 保证它们修改的是各自的文件副本，不会互相干扰。
