...
# 将需求转化为 FD——Claude 自动填写四个维度并写入 fd/FD-001-xxx.md，
# 同时更新 FEATURE_INDEX.md
> /fd-new 优化短剧列表首屏渲染时间    # → 生成 FD-001
> /fd-new 添加用户观看历史记录功能    # → 生成 FD-002

# ══ 终端：Planner ═══════════════════════════════════════════
# 加载代码库上下文，识别风险，补全 FD Solution 细节
> /fd-explore FD-001
> /fd-explore FD-002

# 对比两个 FD 的 "Files to Modify"，确认无文件重叠后创建 Worktree
# 有重叠 → 串行；无重叠 → 继续并行
git worktree add ../project-fd001 -b feature/FD-001
git worktree add ../project-fd002 -b feature/FD-002

# ══ 终端 A：Worker 1 ════════════════════════════════════════
cd ../project-fd001 && claude --model claude-opus-4-8
> 实现 FD-001，完成后依次执行 /fd-verify 和 /fd-close

# ══ 终端 B：Worker 2（同时开启，不等 A 完成）═══════════════
cd ../project-fd002 && claude --model claude-opus-4-8
> 实现 FD-002，完成后依次执行 /fd-verify 和 /fd-close

# ══ 等待完成，合并主干 ══════════════════════════════════════
# 无需盯屏——配置附录 B 的空闲通知后，Agent 跑完时终端 Tab 自动反色提示。
cd <你的项目目录>
...
> **并发的关键**：Worker A 和 Worker B 同时启动，互不等待。PM 定义需求，Planner 补全方案，Worker 只管执行——角色不混用，状态通过 `fd/` 目录共享。
