...
1. [设计先行，架构不能外包](#设计先行架构不能外包)
2. [用 CLAUDE.md 或合适的 md 固化约束](#用-claudemd-或合适的-md-固化约束)
3. [把需求写成结构化 Prompt](#把需求写成结构化-prompt)
4. [小步拆解，每步可验证](#小步拆解每步可验证)
5. [用好 Git，随时掌握节奏](#用好-git随时掌握节奏)
6. [必须看代码，微观决策不能都外包，代码量增长后及时重构](#必须看代码微观决策不能都外包代码量增长后及时重构)
7. [管理好 Session 和上下文窗口](#管理好-session-和上下文窗口)
8. [了解哪些任务 AI 不擅长](#了解哪些任务-ai-不擅长)
9. [分清执行类和架构类任务](#分清执行类和架构类任务)
10. [对 Agent 保持高可观测性](#对-agent-保持高可观测性)
11. [保持人类最终裁决权（HITL）](#保持人类最终裁决权hitl)
