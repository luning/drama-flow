...
代码库结构是地基，三个层级是地基之上可独立建设的工程制品，从软到硬排列：

| 层 | 涵盖内容 | 约束强度 | 存在形式 |
|---|---------|---------|---------|
| **知识层** | CLAUDE.md、模块文档（README/MODULE.md）、ADR、EXPERIENCE.md、领域知识 | ★★☆☆☆ | Markdown，渐进披露，随代码 colocate |
| **约束层** | import-linter、pyproject.toml、CI、Hooks | ★★★★☆ | 可执行规则 + 管线拦截 |
| **执行层** | Skills、Sub-Agents、MCP、scripts、测试套件、可观测性接口 | ★★★★☆ | 注入 Prompt + 隔离执行 + 反馈回路 |

核心规律：**约束越往下越硬（靠工具强制而非 Agent 自觉）。** 成熟的 Harness 在三层之间形成纵深防御，不会只依赖某一层。
...
知识层的内容以 Agent 可读的 Markdown 存在，涵盖架构、规范、决策、任务教训等各类上下文。核心机制是**渐进披露（Progressive Disclosure）**：任何时刻只加载当前任务所需的信息，无关内容不占推理窗口。
...
```
系统级（CLAUDE.md / README.md）
  └── 顶层架构、核心技术栈选型理由（Decision Log）、全局约束

模块级（每个子目录的 README.md / MODULE.md / EXPERIENCE.md）
  └── 职责定位、对外接口、依赖关系、特殊约束、该模块的历史教训
...
模块级知识 colocate 在源码旁，随代码同步演化。README 是最基础的载体：
...
代码级类型提示是另一种形式的模块级知识，不占文档空间却显著降低 Agent 推理成本：
...
并非所有知识都应该预先写好。以任务教训为例：先让 Agent 直接尝试，真的反复犯同一类错误时再提炼——过早写只会制造维护负担。
...
知识文件不必手写——**Git 提交历史本身就是最真实的经验库**。它记录了团队在真实约束下每类任务实际动了哪些文件，比人工回忆更可靠，也没有主观美化。

关键在于按**任务**而非提交聚合：一个功能往往横跨多个提交（初始实现、Review 修改、补测试），要把同一任务的多个提交作为整体分析，才能得到完整的变更集；逐提交分析只会得到碎片。
...
# 然后让 Claude 分析：
# "每个合并记录代表一个完整任务。识别出最常见的几类任务类型，
#  对每类任务列出：① 容易遗漏的必改位置，② 项目特有的隐性约定，
#  ③ 有顺序依赖的关键步骤。跳过凭代码结构能直接推断的内容。"
```

分析结果是候选模式，不是定论——Git 只记录"动了什么"，不记录"为什么动"，因果关系需要人工确认。

**提炼出的是候选项，不是定论。** 以下几类应当直接丢弃：

- **偶然共变**：两个文件碰巧同时被改，但并无因果关系
- **Agent 能力范围内的步骤**：凭代码结构和类型系统就能推断出来，写出来只是噪音
- **可以用设计消除的遗漏**：能从设计上解决的问题，不应靠知识文件打补丁
- **极少触发的场景**：一年用不到一次，等真正遇到时再写

错误的知识比没有知识更危险——Agent 照着错误的清单执行，问题更难排查。

#### 加载机制

知识写好了还不够，要让 Agent 在正确时机加载。核心原则是**保持 CLAUDE.md 精简**，通过索引文件实现渐进披露：

```
CLAUDE.md（一级指针，始终精简）
  └── "如果任务触发以下场景，查阅对应知识文件，见 .claude/INDEX.md"

.claude/INDEX.md（二级索引，按需维护）
  ├── 新增 API 路由       → experience/NEW_API_ROUTE.md
  ├── 修改数据模型        → experience/DATA_MODEL_CHANGE.md
  └── 新增 Android 页面   → experience/ANDROID_PAGE.md

.claude/**/*.md（具体知识文件）
```

新增知识只需更新 INDEX.md，不触碰 CLAUDE.md——保持 CLAUDE.md 精简的同时，不限制知识文件的数量。

对于高频且复杂的任务类型，把"加载知识 + 执行任务"整体封装成 Skill，比依赖 Agent 自觉查 INDEX 更可靠——Skill 开头直接 `Read` 对应的知识文件，不依赖 Agent 的主动性。

---

### 2. 约束层 — 可执行规则，而不是口头约定

约束层与知识层的区别在于：知识层靠 Agent 自觉遵守，约束层**编译时或运行时强制拦截**。
