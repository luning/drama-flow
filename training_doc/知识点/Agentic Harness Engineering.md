...
2. [核心概念：Harness 的七个组件](#2-核心概念harness-的七个组件)
3. [一个典型的 Harness 目录树](#3-一个典型的-harness-目录树)
4. [为什么 Harness Engineering 是瓶颈](#4-为什么-harness-engineering-是瓶颈)
   - [4.1 手工工程化的极限](#41-手工工程化的极限)
   - [4.2 自动演化面临的三重障碍](#42-自动演化面临的三重障碍)
5. [AHE 原理：三支柱可观测性驱动的自动演化](#5-ahe-原理三支柱可观测性驱动的自动演化)
   - [支柱 1：组件可观测性](#支柱-1组件可观测性)
   - [支柱 2：经验可观测性](#支柱-2经验可观测性)
   - [支柱 3：决策可观测性](#支柱-3决策可观测性)
6. [演化循环：从失败轨迹到可验证改进](#6-演化循环从失败轨迹到可验证改进)
   - [6.1 外层循环（Algorithm 1）](#61-外层循环algorithm-1)
   - [6.2 案例分析：db-wal-recovery 任务](#62-案例分析db-wal-recovery-任务)
   - [6.3 组件层级选择](#63-组件层级选择)
7. [关键实验发现](#7-关键实验发现)
   - [7.1 主实验：AHE 超越人工设计和自动化基线](#71-主实验ahe-超越人工设计和自动化基线)
   - [7.2 跨基准转移：Harness 具备通用性](#72-跨基准转移harness-具备通用性)
   - [7.3 跨模型转移：+5.1 到 +10.1 pp](#73-跨模型转移51-到-101-pp)
   - [7.4 组件消融：增益在哪？](#74-组件消融增益在哪)
   - [7.5 自归因的可靠性](#75-自归因的可靠性)
8. [在 Claude Code 中的应用](#8-在-claude-code-中的应用)
   - [8.1 可观测性基础：Session JSONL](#81-可观测性基础session-jsonl)
   - [8.2 演化闭环](#82-演化闭环)
   - [8.3 举个例子](#83-举个例子)
   - [8.4 成熟度](#84-成熟度)
   - [8.5 实操建议](#85-实操建议)
