...
**Project → Milestone → Phase 驱动的工作流**：用户初始化项目（`/gsd:new-project`）时自动生成 ROADMAP.md，以里程碑和阶段组织任务。日常使用无需记忆四个命令的流程，而是通过 `/gsd:progress` 查看当前进度和下一步建议，或通过 `/gsd:ns-workflow` 路由到当前阶段所需的操作。内部每个 phase 的完整生命周期确实包含讨论 → 计划 → 执行 → 验证四个环节，但这是工具内部编排的，用户感受到的是"沿 roadmpa 推进"，而非手动执行一套仪式流程。执行阶段会将 phase 内的多个 plan 拆分为波（Wave），依赖分析后决定哪些可以并行、哪些需要串行。

**新鲜上下文隔离**是 GSD 最核心的技术点：每个执行单元拥有独立的上下文窗口（Claude Code 上接近 200K tokens），从项目工件构建而非累积聊天历史。执行 10 个任务不会出现早期决策被遗忘的问题，上下文不会随时间线性膨胀。

**执行优先的上下文工程**（Execution-First Context Engineering）是对上述设计的进一步概括，核心思想是：**上下文不是事先"搜集"的，而是在执行中自然"产生"的**。传统做法是先花大量时间搜集全量上下文再开始编码——上下文越搜集越多，行动越犹豫。GSD 反过来：每个子 Agent 启动时只带刚好够用的最小上下文（如 executor 只读 PLAN.md），执行完毕后将结果沉淀为工件（SUMMARY.md、代码提交），这些工件成为下一阶段的上下文来源。多 Agent 并行的意义也在此——每个子 Agent 拥有独立的新鲜上下文窗口，互不污染，执行完毕后通过工件汇交流结果：
...
上下文在执行中产生，而非在执行前堆砌——这就是"执行优先"的含义。
