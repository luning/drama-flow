# AI Native 项目实战训练营 —— 基于 Harness Engineering 方法论

## 课程简介

本课程以**短剧应用（DramaFlow）**为核心载体，带领学员完整经历从产品设计、视觉设计、架构搭建到三轮迭代开发、集成测试、代码审阅与重构的全流程，同时还包括已有功能的改版升级。课程通过实战过程将 Harness Engineering 方法论的核心组成——Spec 驱动开发（SDD）、Agent 友好型架构、Skills & MCP 设计等——全部落实在真实项目步骤中，让学员"做中学、学中做"。
...
- 掌握 Harness Engineering 在项目中的系统应用
- 掌握 SDD 工作流，可直接带回团队落地
- 掌握 Skills 和 MCP 的设计与实现方法，适用场景
...
| H5（WebView 内嵌）| Vue3 + Vite + TypeScript | 内嵌于 WebView，内容类页面可热更新无需发版 |
| 后端 API（主）| Python 3.10+ + FastAPI + SQLAlchemy | AI 友好，语法简洁，自动生成 Swagger 文档 |
| 后端 API（辅）| Java Spring Boot | 关键 API 片段演示，不维护完整项目 |
| 数据库 | SQLite | 零维护成本 |
...
本课程围绕 Harness Engineering 将知识点组织为 **5 条知识体系**，每条线在项目的不同阶段在用到的时候分层讲解，而非一次性灌输。

| 主线 | 对应知识点 |
|------|-----------|
| **规格驱动** | PRD→Spec、增量 Spec、OpenSpec等SDD工具、自定义SDD |
| **Agent 友好架构** | Harness 项目空间，文档、经验、约束、执行层设计 |
| **技能设计** | Skill 设计、MCP 设计 |
| **测试自愈** | Spec驱动测试，行为级测试、Agent驱动验收、RCA |
...
### 1. AI Native 认知、编程经验与 Harness 工程（约1.5h）

> 目标：建立"AI Native"思维，理解 Harness 工程体系，掌握核心实战经验

**1.1 AI 能力演进的工程含义（10min）**
...
**1.3 Harness 工程系统介绍（35min）**

- **什么是 Harness**：模型只是拼图的一块，模型外部的工程组件共同组成 Agent 的 Harness，同等决定任务质量。
- **Harness 目录树纵览**：展示一个典型项目完整Harness全貌
- **从五个层次理解Harness构筑的完整体系**：架构层 -> 文档层 -> 经验层 -> 约束层 -> 执行层
- **Harness 的团队治理与 Git 管理**：团队共治 Harness 时的现实思考
...
- 工具原理说明：Claude Code、OpenSpec 与 Spec 的互动关系
- 与 Prompt SDD 的对比：什么样的团队 / 项目值得引入工具化 SDD；说明两者并存，迭代开发时学员可自选使用

**3.5 [实践步骤] 数据模型设计（15min）**

- 任务：Drama / Episode / User / WatchRecord 数据模型 + SQLite 建表脚本 + 测试种子数据（SQLAlchemy ORM + 初始化脚本）
- **自我尝试**：第一轮不读 SPEC.md 直接让 AI 生成模型 → 第二轮加入约束重新生成 → 对比差异，体会 SDD 如何约束输出质量
- 重点关注：WatchRecord 的字段设计（播放进度、时间戳），为迭代 2 播放进度持久化打基础
...
- 开发策略：先生成 Auth Service + API 路由（后端），再补充 Android UI 逻辑（多轮约束生成）
- **Java 辅助演示**：同一登录接口用 Java Spring Boot 实现一遍，对比 AI 在两种语言下的代码生成差异

**4.2 [实践步骤] Home 首页开发（40min）**

- 任务：Python 后端 `/api/dramas` 接口 + Vue3 H5 首页（Banner 轮播 + 分类 Tab + 剧集列表 + Pinia 状态管理）+ Android WebView 加载

**4.3 [实践步骤] 实现第一个 Skill（25min）**

- **Skill 设计方法与适用场景**：什么场景应该封装成 Skill？颗粒度如何把握？Skill目录结构，参数设计原则（最小化、自包含）
...
**6.6 [实践步骤] Agent 自动化验收 / 探索性测试（25min）**

- **解读测试框架**：了解 test-agent（基于 ADB）的 CLI 命令、核心模块及原理
...
**6.7 [实践步骤] 实现 MCP Server（25min）**

- **MCP 设计方法与适用场景**
- **场景**：内容运营通过 AI 查询剧集数据和用户观看统计，无需直接接触数据库——既能用自然语言提问，又限制了原始数据的访问权限
- **任务**：为 DramaFlow 实现一个只读 MCP Server，暴露两个工具：
  - `get_drama_stats`：查指定剧集的播放量、评分、平均完播率
  - `get_user_watch_summary`：查用户近期观看记录（最近观看剧集、完播数量、累计时长）
- **演示重点**：MCP 工具定义 → 在 Claude Code 中注册 → 用自然语言触发工具调用（"查一下剧集 ID 为 3 的播放量和完播率"，"用户 1 最近在看哪些剧？"）
- **与 Skill 对比**：Skill 编排 Agent 的执行流程，MCP 向 Agent 暴露受控的系统能力——两者解决不同层次的问题，可以组合使用

---

### 7. Day 2 上午分享总结（15min）
...
**8.2 [规格驱动] 引入 GSD —— 执行优先的上下文工程（20min）**
...
**9.1 通过 Agentic Harness 工程自动优化 Harness（20min）**

- 自动演化三重障碍：异质动作空间、轨迹信息淹没、效果难以归因。核心洞察：**瓶颈在可观测性，不在 Agent 能力**
- AHE 三支柱：组件可观测性 → 经验可观测性 → 决策可观测性
- 关键结论：增益在 tools / middleware / memory，不在 system prompt；Harness 事实性结构可跨模型转移，散文式策略不可转移
- Claude Code 落地：Session JSONL → 提取失败信号 → LLM 分析根因 → harness 变更 → 频次对比验证

**9.2 Harness 工程疑难杂症应对（25min）**

- **高频问题**：遗留代码无测试，Agent 过度重构，规范臃肿，多步骤错误累积，长会话规则失效等等
- **设计反模式**：过时假设、重叠规则、防御性膨胀、万能 Prompt 补丁、推理替代脚本等等
- **维护原则**：区分不可替代的架构约束和可淘汰的重复提示，换模型即跑裸奔基线退役无效组件，修复Harness的步骤

**9.3 SDD 生态全景与扩展策略（15min）**
...
- SDD 的定制与扩展：认清 SDD 做的事，什么时候需要扩展，三种扩展方式
