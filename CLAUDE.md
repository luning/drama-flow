...
以下 Skill 定义在 `.claude/skills/` 中，对应场景时按需读取：

| Skill | 触发场景 | 用途 |
|-------|---------|------|
| [seed-data](.claude/skills/seed-data/SKILL.md) | 添加/重置测试数据 | 幂等导入 5 部剧集 + 50 集测试数据 |
| [spec-validate](.claude/skills/spec-validate/SKILL.md) | 检查 AC 覆盖率 | 对比 SPEC.md 验收标准与测试覆盖 |
| [cr-refactor](.claude/skills/cr-refactor/SKILL.md) | 代码审查/重构 | 输出 CR 清单和重构建议 |
| [preview-deploy](.claude/skills/preview-deploy/SKILL.md) | 启动预览环境 | 启动 FastAPI 开发服务器供验收 |
| [security-review](.claude/skills/security-review/SKILL.md) | 安全检查 | 扫描 JWT/Token/CORS/WebView 等安全风险 |
| [test-run](.claude/skills/test-run/SKILL.md) | 运行测试 | 执行 pytest 并输出结构化报告 |
| [db-reset](.claude/skills/db-reset/SKILL.md) | 重置数据库 | 删除 dramaflow.db 并重新 seed |
